...
from PyQt5.QtGui import QColor, QFont, QIcon, QPainter, QPen, QPixmap, QRegExpValidator
...
def _depo_app_icon() -> QIcon:
    ...
    p.setPen(QPen(QColor("#b45309"), 2))
    p.drawRoundedRect(6, 10, 52, 44, 10, 10)
    p.setPen(QPen(QColor("#312e81"), 3))
    p.drawLine(14, 22, 50, 22)
    p.drawLine(14, 32, 50, 32)
    p.setBrush(QColor("#6366f1"))
    p.setPen(Qt.NoPen)
    p.drawEllipse(44, 6, 14, 14)
    p.end()
    return QIcon(pm)
...
def run() -> None:
    ...
    app.setWindowIcon(_depo_app_icon())
